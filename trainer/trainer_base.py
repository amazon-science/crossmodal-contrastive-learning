...
from utils_collection import general, metrics, yaml_config, config_tools as congt
from utils_collection.config_tools import MetricComparisonConst
from utils_collection import general, metrics as metric
...
class BaseTrainer:
    ...
    def __init__(
            self, cfg, exp_group: str,
            exp_name: str, run_name: str, train_loader_length: int, model_type: str, *,
            log_dir: str = "experiments", log_level = None, logger = None,
            reset: bool = False, is_test: bool = False):
        ...
        self.state = congt.BaseTrainerState()
        ...
        if logger is None:
            if log_level is None:
                self.log_level = congt.LogLevelsConst.INFO
            else:
                self.log_level = log_level
            self.logger = congt.create_logger(congt.LOGGER_NAME, log_dir=self.exp.path_logs, log_level=self.log_level)
        else:
            self.logger = logger
        ...
        yaml_config.dump_yaml_config_file(self.exp.path_base / 'config.yaml', self.cfg.config_orig)
    ...
    def check_is_test_epoch(self) -> bool:
        """
        Check if test is needed at the end of training epochs.

        Returns:
            Whether or not test is needed.
        """
        # check if we need to validate
        do_test = (self.state.current_epoch % self.cfg.test.test_freq == 0 and self.cfg.test.test_freq > -1
                  and self.state.current_epoch >= self.cfg.test.test_start)
        # always validate the last epoch
        do_test = do_test or self.state.current_epoch == self.cfg.train.num_epochs
        return do_test
    ...
    def close(self) -> None:
        ...
        """
        congt.remove_handlers_from_logger(self.logger)
        self.metrics.close()
    ...
    def hook_pre_train(self) -> None:
        ...
        self.logger.info("Start Training Models ")
    ...
    def hook_post_step(
            self, epoch_step: int, loss: th.Tensor, lr: float, additional_log = None) -> bool:
        """
