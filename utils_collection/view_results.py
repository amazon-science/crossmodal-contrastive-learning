...
from utils_collection import config_tools as confgt
from utils_collection.experiments import ExperimentFilesHandler
from utils_collection.metrics import DEFAULT_METRICS, DefaultMetricsConst, PrintGroupConst, PrintMetric
from utils_collection.config_tools import LOGGER_NAME, TrainerPathConst
    ...
    for exp_group, exp_names in exp_groups_names.items():
        ...
        for exp_ident in exp_names:
            ...
            if not epoch_file.is_file():
                logger.debug("Skipping metrics for {}.".format(exp_name))
                continue
            ...
            for key in (DefaultMetricsConst.TIME_STEP_FORWARD, DefaultMetricsConst.TIME_STEP_BACKWARD,
                        ...
                        DefaultMetricsConst.TIME_STEP_TOTAL):
            ...
            # for key in (DefaultMetricsConst.PROFILE_RAM_USED):
...
def display_table(lines: List[List[str]], header: List[str] = None, sep_line="---", use_colors=True,
    ...
    lines_len = confgt.np_str_len(lines_arr)
