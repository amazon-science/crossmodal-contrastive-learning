...
def compute_scores(emb1, emb2, key1: str, key2: str, print_fn: Callable = print) -> (
    ...
    print_fn(retrieval_results_to_str(res1, key1[:-4]))
    print_fn(retrieval_results_to_str(res2, key2[:-4]))
    result_str = f"{key1[:-4]}{key2[:-4]} ({num_points}) in {timer() - start_time:.3f}s, "
