    ...
    for vid_gp in groups:
        ...
        if str(vid_gp_id) not in vid_h5:
            ...
            for clip_id in clip_ids:
                ...
                # elif 'face' in args.modality_list:
                #     video_decord = decord.VideoReader(video_path,
                #                                       ctx=cpu(0),
                #                                       width=224,
                #                                       height=224,
                ...
                if 'action' in args.modality_list:
                    ...
                    for pointer in range(0, num_frames, stride):
                        frames_single = clip_input[:, pointer:pointer +
                                                   sliding_window_size, :, :]
                        ...
                        if frames_single.shape[1] == sliding_window_size:
                            ...
                            frames_collector.append(frames_single)
                        if len(frames_collector) == batch_size:
                            batch_input = torch.stack(frames_collector, dim=0)
                            feat = expert_models['action'](batch_input.cuda())
                ...
                if 'flow' in args.modality_list:

                    clip_features = []
                    # clip_object_featues = []
                    num_samples = 8
                    num_frames = len(video_decord)
                    short_side_size = 256
                    crop_size = 256
                    transform_fn = video_transforms.Compose([
                        video_transforms.Resize(short_side_size, interpolation='bilinear'),
                    ...
                    clip_input = transform_fn(frames)
                    # print(clip_input.shape)
                    # clip_input = clip_input.permute(1, 2, 3, 0)
                    tick = len(clip_input) / float(num_samples)
                    offsets = np.array([
                        int(tick / 2.0 + tick * x) for x in range(num_samples-1)
                    ])

                    avg_pool2d = nn.AvgPool2d((4, 4))
                    feat_extract = 1
                    for i_frm in offsets:
                        input1 = torch.unsqueeze(
                            clip_input[:, i_frm, :, :], dim=0
                        ) 
                        input2 = torch.unsqueeze(
                            clip_input[:, i_frm+1, :, :], dim=0
                        ) 
                        if feat_extract:
                            # extract flow feature
                            flow_feat = expert_models['flow'](input1.cuda(), input2.cuda(), feat=True)
                            feat_flow = avg_pool2d(flow_feat).view(-1, 529)
                            clip_features.append(feat_flow)
                            
                        else:
                            # compute optical flow
                            from thirdparty.flow_pwcnet import flow_viz
                            import cv2
                            print(input2.shape)
                            flow_out = expert_models['flow'](input1.cuda(), input2.cuda(), feat=False)
                            tenFlow = 20.0 * torch.nn.functional.interpolate(input=flow_out,
                                                                            size=(crop_size, crop_size),
                                                                            mode='bilinear',
                                                                            align_corners=False)

                            flow_tensor = tenFlow[0, :, :, :].cpu()
                            flow_tensor = flow_tensor.permute(1,2,0).cpu().numpy()
                            flow_image = flow_viz.flow_to_image(flow_tensor)
                            cv2.imwrite('./pred_flow_m1.png', flow_image[:,:,::-1])
                            x= s
                ...
                if 'audio' in args.modality_list:
                    ...
                    eps = 1e-6
                    # video_path = 'thirdparty/action_ircsn_res152/abseiling_k400.mp4'
                    y_ts, y_sr = librosa.load(video_path, sr=sr)

                    # pre-defined hyper-parameters
                    hypm = [256, 512, 512,
                            80]  #All hyp should be fixed. 257 * hop length

                    clip_features = []
                    # compute a mel-scaled spectrogram
                    S = librosa.feature.melspectrogram(y=y_ts,
                                                       sr=sr,
                                                       n_fft=hypm[1],
                                                       win_length=hypm[2],
                                                       hop_length=hypm[0],
                                                       ...
                                                       center=False)
                    logS = np.log(S + eps)
                    # mean subtraction and std division. The mean and std are computed on AudioSet
                    logS = (logS - (-7.03)) / 4.66
                    # print(logS.shape)
                    input_batch = torch.unsqueeze(torch.unsqueeze(
                        torch.from_numpy(logS), dim=0),
                                                  dim=0)
                    feat = expert_models['audio'](input_batch.cuda(
                    ))  #Before global average pooling - check the model
                    #print("out feat shape: ", feat.shape)
                    # print('squeeze dim: ', feat[:,:,:, 1].squeeze().reshape(1, -1).shape)

                    num_samples = 6  # we only select 6 audio features per video at maximum
                    if feat.shape[3] < num_samples:
                        offsets = range(feat.shape[3])
                    else:
                        tick = feat.shape[3] / float(num_samples)
                        offsets = np.array([
                            int(tick / 2.0 + tick * x)
                            for x in range(num_samples)
                        ])

                    for i_audio in offsets:
                        clip_features.append(
                            feat[:, :, :, i_audio].squeeze().reshape(1, -1))

                ## Face - MTCNN
                if 'face' in args.modality_list:
                    ...
                    for i_frm in offsets:
                        ...
                        # img = Image.fromarray(clip_input[:, i_frm, :, :], 'RGB')
                        try:
                            batch_boxes, batch_probs, batch_points = expert_models['face'].detect(clip_input[i_frm], landmarks=True)
                            # Select faces
                            threshold = 0.8
                            selected_boxes = []
                            selected_probs = []

                            for fid, fscore in enumerate(batch_probs):
                                if fscore >= threshold:
                                    selected_boxes.append(batch_boxes[fid])
                                    selected_probs.append(batch_probs[fid])

                            vis = False
                            faces = expert_models['face'].extract(clip_input[i_frm], selected_boxes, save_path="./", vis=vis)
                            # print('Cropped face images: ', faces.shape)

                            # Get pretrained face embedding model
                            resnet = InceptionResnetV1(pretrained='vggface2').eval()
                            # print('Build FaceNet model for feature extraction.')

                            # Calculate embedding, the dimension should be N x 512, where N is the number of predicted faces
                            feat_frame = resnet(faces)
                            ...
                            clip_faces.append(frame_features)
                        except:
                            continue
                        # print(frame_classes)
                    ...
                    if len(clip_features) == 0:
                        count_no_face += 1
                        print("====> No face found!!!: ", count_no_face)
                        clip_features.append(torch.zeros(1,512).reshape(1, -1))
                        frame_features = torch.zeros(1,512).reshape(1, -1)
                        clip_faces.append(frame_features)
                ...
                if 'object' in args.modality_list:
                        ...
                        try:
                            ...
                            if save_output == 1:
                                ...
                                for i in range(len(pred_boxes_threshold)):
                                    ...
                                    cv2.rectangle(img, pred_boxes_threshold[i][0], pred_boxes_threshold[i][1],
                                                color=(0, 255, 0), thickness=rect_th)
                                    # Write the prediction class
                                    print(pred_class_threshold[i])
                                    cv2.putText(img, pred_class_threshold[i], pred_boxes_threshold[i][0],
                    ...
                    if len(clip_features) == 0:
                        clip_features.append(torch.zeros(1,256).reshape(1, -1))
            ...
            if 'face' in args.modality_list:
                gp_face_folder = os.path.join(data_dir, 'face')
                os.makedirs(gp_face_folder, exist_ok=True)

                gp_face_file = os.path.join(gp_face_folder, str(vid_gp_id) + '_face.pickle')
                with open(gp_face_file, 'wb') as f:
                    group_face_all = {}
                    group_face_all['features'] = group_faces
                    pk.dump(group_face_all, f, pk.HIGHEST_PROTOCOL)
                # print("vidH5: ", vid_h5)
